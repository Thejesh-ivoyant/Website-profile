  ...
  return (
    ...
    >
      <section className="section-title section-title-blue">
        <p role="complementary">Technology Expertise</p>
      </section>
      <section
        className="px-4 py-2"
        role="complementary"
        aria-label="List of all the technologies that we usually adopt and there are much more"
      >
        <div className="flex flex-col space-y-4 gradient-bottom">
          {Array.from({
            ...
            <div key={row} className="flex flex-row w-full justify-evenly gradient-top py-8">
              {TechnologyExpertiseLogos.slice(row * columnCount, (row + 1) * columnCount).map(
                (logo, index) => (
