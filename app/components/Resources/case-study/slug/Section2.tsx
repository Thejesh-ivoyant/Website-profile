...
const Section2 = ({ data }: { data: any }) => {
  ...
  const gradientStyle = {
    background: `linear-gradient(180deg, rgba(0, 0, 0, 0.60) 0%, rgba(0, 0, 0, 0.60) 66.95%, rgba(0, 0, 0, 0.00) 152.46%), url(${data?.section2Bg?.data?.attributes.url}) lightgray 50% / cover no-repeat`,
  ...
  return (
    <section className="relative flex justify-center md:flex-row flex-col h-fit lg:min-h-[37.5rem] w-full bg-[#1B0740] ">
      ...
      >
        <div className="bg-[#170D26] font-montserrat italic font-medium w-fit h-fit pb-2">
          {data?.section2MiniTag}
        </div>
      ...
      <div
        className="flex flex-1 flex-col  bg-black md:min-h-full md:m-0 m-3"
        style={gradientStyle}
      >
