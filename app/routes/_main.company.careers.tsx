...
export async function loader() {
  const componentRes = await fetchData('/api/career?populate=s4_cards.bgImage,s2_whyJoinUs.bgImage')
  ...
  const JobDesc = jsonParsed?.data?.career?.data?.attributes?.job_descriptions?.data?.map(
    (item: any) => ({
  ...
  return defer({
    heroBgImageURl: jsonParsed.data?.career.data?.attributes.heroImage.data?.attributes.url,
    heroTitle,
    heroDescription,
    s2_title,
    s2_description,
